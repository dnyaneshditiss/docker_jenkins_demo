pipeline {
    ...
    environment {
        GIT_REPOSITORY_URL = 'https://github.com/dnyaneshditiss/docker_jenkins_demo.git'
        DOCKER_IMAGE_NAME = 'dnyaneshditiss/docker_jenkins_demo'
        IMAGE_TAG = '1.0'
    }
                ...
                script {
                    try {
                        withCredentials([usernamePassword(credentialsId: 'my-docker-hub-credentials-id', 
                                                         usernameVariable: 'dnyaneshditiss', 
                                                         passwordVariable: 'Pass@12345')]) {
